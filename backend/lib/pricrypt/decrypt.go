...
type PublicKeyDecrypt struct {
	PublicKey []byte //公钥
	CiperText []byte //密文
	PlainText string //明文
}

func (p *PublicKeyDecrypt) PublicDecrypt() error {
	pub, err := parsePublicKey(p.PublicKey)
	if err != nil {
		return err
	}
	pbyte, err := decrypt(pub, p.CiperText)
	if err != nil {
		return err
	}
	p.PlainText = string(pbyte)
	return nil
}
...
func decrypt(pub *rsa.PublicKey, data []byte) ([]byte, error) {
